        ...
        public List<TaskDTO> getTasksByGroup(UUID groupId) {
                return taskRepository.findAll().stream()
                                .filter(task -> task.getRequirement() != null
                                                && task.getRequirement().getProjectGroup().getGroupId().equals(groupId))
                                .map(this::toTaskDTO)
                                .collect(Collectors.toList());
        }
        ...
        public void assignTask(UUID taskId, UUID studentId, UUID requesterId) {
                ...
                Student student = studentRepository.findById(studentId)
                                .orElseThrow(() -> new RuntimeException(
                                                "Assigned student does not exist: " + studentId));
        ...
        private TaskDTO toTaskDTO(Task task) {
                ...
                long commitCount = vcsCommitRepository.findAll().stream()
                                .filter(c -> c.getRequirement() != null && task.getRequirement() != null
                                                && c.getRequirement().getRequirementId()
                                                                .equals(task.getRequirement().getRequirementId())
                                                && student != null && c.getStudent() != null
                                                && student.getId().equals(c.getStudent().getId()))
                                .count();
